...
def evaluate(logdir, save_predict=False):
    ...
    # model = get_mutual_model(cfg).to(device)
    model = load_ckpt(logdir, model, kind='e')
    ...
    #创建目录来保存模型的预测结果
    save_path = os.path.join(logdir, 'predict_1')
    ...
    with torch.no_grad():
        ...
        for i, sample in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            # predict = predict[2].max(1)[1].cpu().numpy()
        ...
        for i, sample in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            for scale in eval_scales:
                ...
                if eval_flip:
                    ...
                    prob = F.softmax(out, 1)
                    predicts += prob
            predict = predicts[0].max(1)[1].cpu().numpy()
            label = label.cpu().numpy()
            running_metrics_val.update(label, predict)
...
if __name__ == '__main__':
    ...
    #####
    parser.add_argument("--logdir", type=str, help="run logdir", default="/media/wby/shuju/Seg_Water/Udw3/sam/udw/run/2025-06-19-08-38-SAM_2_5Prompt1234_NEW")    #########
