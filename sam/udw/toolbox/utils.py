...
def save_ckpt(logdir, model,kind, cur_epoch=1):
    if kind=='end':
        ...
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'model_A_best.pth'))
    elif kind == 'end_A':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'model_A_end.pth'))
    elif kind == 'best_B':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'model_B_best.pth'))
    elif kind == 'epoch':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'model_{}.pth'.format(cur_epoch)))
...
def load_ckpt(logdir, model, kind):
    if kind=='end':
        ...
        save_pth = os.path.join(logdir, 'model_A_best.pth')
        model.load_state_dict(torch.load(save_pth))
    elif kind == 'end_A':
        save_pth = os.path.join(logdir, 'model_A_end.pth')
        model.load_state_dict(torch.load(save_pth))
    elif kind == 'best_B':
        save_pth = os.path.join(logdir, 'model_B_best.pth')
        model.load_state_dict(torch.load(save_pth))
    elif kind == 'end_B':
        save_pth = os.path.join(logdir, 'model_B_end.pth')
        model.load_state_dict(torch.load(save_pth))
    elif kind == 'e':
        save_pth = os.path.join(logdir, 'model_95.pth')
        model.load_state_dict(torch.load(save_pth))
    return model
